# CodeGraph Retrieval Quality Audit (A/B benchmark)

Maintainer-only workflow for the vendored `vendor/codegraph` submodule. Read this when
asked to test, benchmark, audit, or validate a codegraph version (the local dev build
or a published npm version) against a language's repos, or when the user runs
`/agent-eval`.
