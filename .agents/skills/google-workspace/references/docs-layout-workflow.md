...
This is a skill reference — a reusable procedure that guides AI agents through complex multi-step workflows.
...
This file is a reference of the `google-workspace` skill; load it from that skill when creating or
editing a formatted Google Doc (`google-workspace` is symlinked into each harness's skill root by
`aa connect`, so no manual copy is needed). If you are using it outside this pack, copy it into the
harness's own skill directory and keep the reference link intact from the skill that uses it.
Agents that follow these patterns produce well-formatted Google Docs on the first attempt.
...
```python
mcp__workspace__export_doc_to_pdf(document_id="<doc_id>")
```
