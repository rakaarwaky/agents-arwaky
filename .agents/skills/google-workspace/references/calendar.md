...
| recurrence | array of strings | no | | RFC 5545 rules, e.g. `["RRULE:FREQ=WEEKLY;COUNT=10"]` |
...
**Recurring events:** with `recurrence` set, `start_time`/`end_time` must be wall-clock
(`YYYY-MM-DDTHH:MM:SS`, no `Z`, no `+07:00` offset) plus `timezone`; otherwise the server returns
400 "Missing time zone definition". See [pitfalls.md](pitfalls.md).
