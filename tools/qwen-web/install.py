...
def main() -> int:
    ...
    ensure_bin_home()
    print(f">>> Installing qwen-web launchers to {bin_home()}...")
    for launcher in ['qwen-web-arwaky', 'qwa', 'qwen-web-cli', 'qwen-web-mcp', 'qwc']:
        target = bin_home() / launcher
        content = "#!/usr/bin/env python3\nimport os, sys\nfrom pathlib import Path\nroot = Path(os.environ.get(\"AGENTS_ARWAKY_ROOT\", str(Path.home() / \"agents-arwaky\")))\nos.execvpe(\"uv\", [\"uv\", \"run\", \"--directory\", str(root / \"internal/qwen-web-arwaky\"), \"qwen-web-arwaky\", *sys.argv[1:]], os.environ.copy())\n"
        target.write_text(content, encoding="utf-8")
        target.chmod(0o755)
    print(f">>> Successfully installed qwen-web -> {bin_home()}")
    return 0
