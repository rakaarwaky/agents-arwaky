...
VENDOR_DIR = ROOT / "vendor/codegraph"
TARGET_DIR = data_home() / "codegraph"
LAUNCHER = bin_home() / "codegraph-mcp"
...
import os, sys
from pathlib import Path
data = Path(os.environ.get("XDG_DATA_HOME", str(Path.home() / ".local/share"))) / "codegraph"
...
def main() -> int:
    if not VENDOR_DIR.exists() or not (VENDOR_DIR / "package.json").exists():
        ...
        return 1
    print(f">>> Building codegraph into {TARGET_DIR}...")
    build()
    install_runtime()
    install_launcher()
    print(f">>> Successfully installed codegraph -> {LAUNCHER}")
    return 0
