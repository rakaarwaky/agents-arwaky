...
Two harness facts drive the invariants below. Qwen Code scans exactly *one*
directory level under a skills root, so a skill must sit at
``<category>/<skill>/SKILL.md`` relative to ``skills/`` and every category folder
must itself be registered as a root. And every frontmatter ``description:`` is
injected into every session prompt, so the pack's descriptions are a permanent
token cost, not just documentation.

``aa skill install`` writes ``.arwaky-skill.json`` next to each provisioned
``SKILL.md`` so ``--prune`` can tell its own output from hand-written skills.
"""
...
def prune_provisioned(base: Path, pack_root: Path) -> list[str]:
