...
VENDOR_DIR = ROOT / "vendor/anytype-mcp"
TARGET_DIR = data_home() / "anytype-mcp"
...
import os, sys
from pathlib import Path
data = Path(os.environ.get("XDG_DATA_HOME", str(Path.home() / ".local/share"))) / "anytype-mcp"
...
def main() -> int:
    if not VENDOR_DIR.exists() or not (VENDOR_DIR / "package.json").exists():
        ...
        return 1
    print(f">>> Building anytype-mcp into {TARGET_DIR}...")
    build()
    install_runtime()
    install_launcher()
    print(f">>> Successfully installed anytype-mcp -> {LAUNCHER}")
    return 0
