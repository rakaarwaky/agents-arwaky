...
VENDOR_DIR = ROOT / "vendor/ponytail"
TARGET_DIR = data_home() / "ponytail"
...
import os, sys
from pathlib import Path
data = Path(os.environ.get("XDG_DATA_HOME", str(Path.home() / ".local/share"))) / "ponytail"
...
def main() -> int:
    if not VENDOR_DIR.exists() or not (VENDOR_DIR / "package.json").exists():
        ...
        return 1
    print(f">>> Building ponytail into {TARGET_DIR}...")
    build()
    install_runtime()
    install_launcher()
    print(f">>> Successfully installed ponytail -> {LAUNCHER}")
    return 0
