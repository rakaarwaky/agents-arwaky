...
as a Hermes agent — the paths and tool names below are Hermes-specific. Under another
harness, use that harness's own plan mode.
...
> **For Hermes:** Implement this plan task-by-task — either through the
> `coding-agent-delegation` skill (one headless CLI run per task) or Hermes'
> `delegate_task` with a fresh subagent per task.
...
**"Plan complete and saved. Ready to execute task-by-task — fresh subagent (or fresh
CLI run) per task with two-stage review (spec compliance then code quality). Shall I
proceed?"**
