...
Usage: python3 skill_suite_audit.py <path/to/skills>   # e.g. ~/.hermes/profiles/currie/skills
Read-only. Skips .hub/ and .curator_backups/. Reads use/view counts from <root>/.usage.json
(keys: use_count, view_count, patch_count, last_used_at -- no load_count).
"""
import json
import pathlib
import re
...
def main():
    ...
    for p in sorted(root.rglob("SKILL.md")):
        ...
        name = str(fm.get("name") or pathlib.Path(rel).parts[0])
        ...
        if not u.get("use_count") and not u.get("view_count"):
            flags.append("UNUSED")
        rows.append((rel, name, flags, hits, u.get("use_count", 0), u.get("last_used_at", "")[:10]))
