...
1. **Find the real process name & port first** — `ss -ltnp | grep <port>` and read the process. Don't assume the app from the port number alone (e.g. port 20128 is the "9Router" gateway; deleting the wrong one breaks the user's setup).
...
3. **Pass the headless/background flag** if the app has one (e.g. `9router --port 20128 --no-browser --skip-update` under systemd — no `--tray`, which backgrounds the main PID). Without a headless path, a TTY-less parent often self-exits.
...
- **`cd` into a deleted dir**: if you just removed `~/.9router` the shell's cwd may be invalid — always pass `workdir=/home/<user>` to terminal calls after a destructive rm.
