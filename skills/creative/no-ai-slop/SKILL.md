...
- User shares a draft and wants it clearer, more direct, more opinionated, or "less AI-sounding".
- User asks whether a piece of writing reads as AI slop, or asks to audit/scan/flag without rewriting.
- User invokes `/no-ai-slop` (with or without a draft attached).

Don't use for: writing brand-new content from a prompt (this is an editor, not a generator), or code review.
...
4. For an edit, make the minimum effective changes, then check the edited draft against the Eval checklist below yourself.
