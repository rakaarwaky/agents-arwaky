...
> `create-*` skill that owns that layer.
>
...
> `internal/lint-arwaky/RULES_AES.md` and `internal/lint-arwaky/lint_arwaky.config.yaml` —
...
2. **Route every fix to its owning skill.** `{layer}` ∈ `taxonomy`, `utility`, `contract`,
   `capabilities`, `agent`, `surface`, `root` — the merged skill name (e.g. `create-contract`),
   whose HOW-TO carries the language templates.
3. **Auto-fix only AES101 / AES203 / AES304 via `fix`.** AES201 and AES205 are structural —
   manual, through `create-contract` (or the layer's `create-*`).
...
6. **`test` / `bench` are not AES layers.** Files under `tests/` and `benches/` follow
   `testing-suite` naming; no AES102 suffix rule applies.
...
the merged skill name (e.g. `create-contract`), whose HOW-TO carries the templates for Python,
Rust, or TypeScript.
