...
>
> **Audience**: Agents and engineers running or remediating lint-arwaky on TS/JS packages.
>
...
5. **Auto-fix only what is auto-fixable.** AES101 rename, AES203 unused import, AES304 bypass
   ...
   interface via DI (`create-contract`).
...
lint-arwaky-cli scan packages/ --format json \
  > ~/.local/share/lint-arwaky/reports/scan_typescript.json
lint-arwaky-cli scan packages/ --format sarif \
  > ~/.local/share/lint-arwaky/reports/scan_typescript.sarif
```
