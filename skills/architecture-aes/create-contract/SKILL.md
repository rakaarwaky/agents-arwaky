...
5. **Does `lint-arwaky-cli scan <contract-dir>` exit 0?**
   - *No* → fix findings, re-scan, then implement with `create-capabilities` or `create-agent`.
...
6. **Verify**: `lint-arwaky-cli scan <contract-dir>` (HOW-TO § Verify), then implement with `create-capabilities` / `create-agent`.
