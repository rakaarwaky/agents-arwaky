# Adding a tree-sitter language to CodeGraph

Maintainer-only workflow for the vendored `vendor/codegraph` submodule. Not loaded by
the `codegraph` skill's normal use — read this when asked to add/support a new
language (e.g. Lua, Elixir, Zig, OCaml) in codegraph, or when the user runs
`/add-lang <language>`.
...
`files`, `question`) so the benchmark workflow in `evaluating.md` can reuse them.
...
- The A/B spawns real **paid** `claude -p` runs (opus, `--max-budget-usd`),
  2 arms × 3 repos. The corpus dir `/tmp/codegraph-corpus` is shared with the
  retrieval benchmark, so clones are reused across runs.
