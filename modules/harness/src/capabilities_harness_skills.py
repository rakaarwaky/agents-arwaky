...
class HarnessSkills(IHarnessProtocol):
    ...
    def provision_skills(self, harness_ids: tuple[str, ...], copy: bool = False,
        ...
        if not pack_root.is_dir():
            log_warn(f"Skill pack not found at {pack_root}; skipping all harnesses.")
            return 0
        skill_files = iter_skill_files(pack_root)
        if not skill_files:
            log_warn(f"Skill pack at {pack_root} is empty; skipping all harnesses.")
            return 0
        failures = 0
    ...
    def _provision_one(self, harness_id, adapter, opts: SkillsOpts, skill_files, pack_root) -> int:
        ...
        if link:
            ...
            if failures:
                return failures
            log_ok(f"Skills root linked: {skills_dir} -> {pack_root}")
            ...
            for sf in skill_files:
                if not _provision_skill(sf, skills_dir, opts.force, opts.dry_run):
                    failures += 1
            if failures:
                log_err(f"{harness_id}: {failures} skill(s) failed to provision.")
            else:
                log_ok(f"{adapter.display} skills provisioned.")
        return failures
...
def _provision_skill(src: Path, dest_base: Path, force: bool, dry_run: bool) -> bool:
    ...
    if dry_run:
        log_sub(f"[DRY-RUN] Would copy skill '{name}' -> {dest_dir}")
        return True
...
def _link_skills_root(dest_root: Path, src_root: Path, force: bool, dry_run: bool) -> int:
    ...
    try:
        ...
        log_err(f"Failed to link skills root {dest_root}: {exc}")
        failures = 1
    return failures
...
def _sync_skill_directories(adapter, dry_run: bool) -> None:
    ...
    if added or dropped:
        log_sub("Restart any running session to pick the list up.")
...
def _sync_skill_hook(adapter, dry_run: bool) -> None:
    ...
    for group in starts:
        ...
        if owned.get("command") == command:
            ...
            return
        if dry_run:
            log_sub(f"[DRY-RUN] Would refresh the '{hook_name}' hook command in {settings_file}")
            return
        owned["command"] = command
        settings_file.parent.mkdir(parents=True, exist_ok=True)
        _write_settings(settings_file, settings)
        log_ok(f"Refreshed the '{hook_name}' hook command in {settings_file}.")
        return
    if dry_run:
        log_sub(f"[DRY-RUN] Would install the '{hook_name}' SessionStart hook in {settings_file}")
        return
    starts.append({
        "matcher": "*",
        "hooks": [{
    ...
    settings_file.parent.mkdir(parents=True, exist_ok=True)
    _write_settings(settings_file, settings)
    log_ok(f"Installed the '{hook_name}' SessionStart hook in {settings_file}.")
