"""Qwen Code (qwencode) harness connector — port of connect/qwencode_adapter.py."""
...
ENV_TARGET = "qwencode"
SKILL_LINK_VERIFIED = True  # headless probe: qwen -p sees symlinked skill dir
...
def _read_settings(settings_file: Path) -> tuple[dict | None, bool]:
    """Parse settings.json, tolerating a fresh install. Never clobber unparseable files."""
    try:
...
def _write_settings(settings_file: Path, settings: dict) -> None:
    """Atomic write: sibling + os.replace (SessionStart hook may hold readers)."""
...
def sync_skill_directories(settings_file: Path, roots: tuple[Path, ...] | list[Path], dry_run: bool) -> None:
    ...
    if dry_run:
        log_sub(f"[DRY-RUN] Would register {len(want)} skill directories in {settings_file} "
...
def sync_skill_sync_hook(settings_file: Path, dry_run: bool) -> None:
    """Install/refresh the SessionStart hook that re-registers skill roots."""
    settings, usable = _read_settings(settings_file)
    if not usable:
        return
    ...
    for group in starts:
        ...
        if owned.get("command") == command:
            ...
            return
        if dry_run:
            log_sub(f"[DRY-RUN] Would refresh the '{SKILL_SYNC_HOOK}' hook command in {settings_file}")
            return
        ...
        return
    if dry_run:
        log_sub(f"[DRY-RUN] Would install the '{SKILL_SYNC_HOOK}' SessionStart hook in {settings_file}")
        return
    starts.append({
        "matcher": "*",
        "hooks": [{
...
def remove_skill_sync_hook(settings_file: Path, dry_run: bool) -> None:
    ...
    if not removed:
        ...
        return
    if dry_run:
        log_sub(f"[DRY-RUN] Would remove {removed} skill sync hook(s) from {settings_file}")
        return
...
def sync_router_provider(settings_file: Path, url: str, dry_run: bool) -> None:
...
class QwencodeConnector(IHarnessConnector):
    """MCP + provider + skill-roots sync in settings.json, env injection.
    ...
    def __init__(self) -> None:
        self._qwen_home = _qwen_home()
        self._settings_file = self._qwen_home / "settings.json"
        self._skills_dir = self._qwen_home / "skills"
    ...
    def connect(self, force: bool, dry_run: bool, mcp_only: bool, skills_only: bool, env_only: bool, copy_skills: bool = False) -> None:
        ...
        if not skills_only and not env_only:
            if dry_run:
                ...
                self._qwen_home.mkdir(parents=True, exist_ok=True)
...
def register() -> dict:
    ...
    return {
        ...
        "connect": QwencodeConnector().connect,
        "disconnect": QwencodeConnector().disconnect,
    }
