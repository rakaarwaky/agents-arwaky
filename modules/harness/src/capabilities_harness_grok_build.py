"""Grok Build harness connector — port of connect/grok_build_adapter.py."""
...
def sync_router_provider(cfg_file: Path, dry_run: bool) -> None:
...
class GrokBuildConnector(IHarnessConnector):
    """MCP merge into config.toml, provider sync, whole-root skill link.
    ...
    def __init__(self) -> None:
        self._grok_home = _grok_home()
        self._cfg_file = self._grok_home / "config.toml"
        self._skills_dir = self._grok_home / "skills"
...
def register() -> dict:
    ...
    return {
        ...
        "connect": GrokBuildConnector().connect,
        "disconnect": GrokBuildConnector().disconnect,
    }
