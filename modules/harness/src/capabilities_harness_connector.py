...
def load_generated_servers() -> dict[str, dict]:
    ...
    return {
        name: {"command": f"{name}-mcp" if name not in _FALLBACK_MCP_COMMANDS else _FALLBACK_MCP_COMMANDS[name]}
...
class HarnessConnector(IHarnessConnector):
    ...
    def _connect_skills(self, harness_id: str, adapter, opts: ConnectOpts) -> int:
        ...
        skills = HarnessSkills(self._adapters)
        return skills.provision_skills((harness_id,), copy=opts.copy_skills,
    ...
    def _connect_router(self, harness_id: str, adapter, opts: ConnectOpts) -> int:
        ...
        if not daemon_running(self._daemon_status_fn):
            _log_warn(
                f"9Router daemon not running; router wiring still applied. "
                f"Start it with 'aa 9router start'."
            )
...
def _connect_router_toml(adapter, url: str, key: str, opts: ConnectOpts) -> int:
    ...
    if not cfg_file.exists():
        ...
        return 0
    from modules.config.src.capabilities_config_engine import load_file, save_file
