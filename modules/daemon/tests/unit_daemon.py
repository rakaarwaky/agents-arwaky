...
class TestNinerouterDaemonManagerInit:
    """Tests for NinerouterDaemonManager initialization."""

    def test_init_default_params(self):
        """UT-DAEMON-017: NinerouterDaemonManager initializes with default params."""
    ...
    def test_init_with_params(self):
        """UT-DAEMON-018: NinerouterDaemonManager accepts root and daemons."""
        from modules.daemon.src.capabilities_9router_daemon import NinerouterDaemonManager

        manager = NinerouterDaemonManager(root=Path("/test"), daemons=None)
        assert manager is not None


class TestNinerouterExecuteDispatch:
    """Tests for NinerouterDaemonManager.execute dispatching."""
...
class TestDaemonOrchestrator:
    ...
    def test_init_stores_managers(self):
        ...
        assert orchestrator._ninerouter is ninerouter
        assert orchestrator._anytype is anytype
        assert "9router" in orchestrator._managers
    ...
    def test_stop_routes_to_manager(self):
        ...
        with patch.object(ninerouter, 'execute', return_value=0) as mock_exec:
            result = orchestrator.stop(DaemonName("9router"))
    ...
    def test_for_unit_resolves_by_name(self):
        ...
        result = orchestrator._for_unit("9router")
        assert result is ninerouter
