"""Workspace (google-workspace-mcp) updater adapter — leaf utility for one tool.

Mirrors the original tools/update/update_workspace.py mechanics: submodule
...
def _write_uv_launcher(name: str, entry: str, root: Path) -> Path:
    """Same `uv run` launcher content the installer writes (updater-owned copy)."""
    ensure_bin_home()
    baked_root = str(root)
...
class WorkspaceUpdaterAdapter:
    """Google Workspace MCP (uv) update sequence."""
