...
The skill feature provisions the agent-skill pack. The skill orchestrator
exposes list, check, show, install, uninstall, and sync — registry
operations (list/show) and pack operations (install/prune/sync + audit),
plus skill-name resolution. The pack lives at
...
### FR-SKILL-003: List and show skills
...
- **Business Rules**: resolution uses the shared skill-name helper; an unknown query is
...
| `SkillOrchestrator.check_skills` | — | `ExitCode` + findings | non-zero on error findings | — | Audit pack loadability |
| `SkillOrchestrator.show_skill` | `query: SkillQuery=''` | `ExitCode` + skill body | non-zero on miss | — | Show one SKILL.md |
| `SkillOrchestrator.install_skills` | `args: SkillArgs` | `ExitCode` | non-zero on copy failure | — | Provision skills into a target |
| `SkillOrchestrator.uninstall_skills` | `args: SkillArgs` | `ExitCode` | non-zero | — | Remove provisioned copies (`--prune` safe) |
| `SkillOrchestrator.sync_skills` | `args: SkillArgs` | `ExitCode` | non-zero | — | Alias: install all / re-sync pack |
...
| root CLI (`aa`) | in | `aa skill` | pass-through |
