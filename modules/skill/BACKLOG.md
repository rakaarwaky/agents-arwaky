...
- Done: protocol collapsed to single `execute`; aggregate renamed to
  `list`/`check`/`show`/`install`/`uninstall`/`sync`; scratch-target sweep
  green (install/show/list/check/uninstall/sync) and FRD/BACKLOG pair
  realigned — `python3 -m modules.root_cli_entry check docs modules/skill`
...
| SKL-07 | FR-SKILL-001, FR-SKILL-002, FR-SKILL-003, FR-SKILL-004, FR-SKILL-005 | Single-method protocol (`execute`) + aggregate rename to `list`/`check`/`show`/`install`/`uninstall`/`sync` | P1 | Done | `python3 -m compileall -q modules/skill modules/shared` clean + `lint-arwaky-cli scan modules/skill` → 0 violations + `python3 -m modules.root_cli_entry check docs modules/skill` → 0 findings at `f87a775`. | @raka | None | 2026-09-23 |
