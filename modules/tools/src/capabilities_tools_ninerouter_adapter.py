...
Implements `IToolsProtocol` (AES403) and exports the `9router`
...
class NinerouterToolsAdapter(IToolsProtocol):
    ...
    def execute(
        ...
        """Dispatch *op* against this file's 9router unit."""
        if spec is None:
            raise ToolUpdateError(f"9router adapter got op={op!r} without a spec")
