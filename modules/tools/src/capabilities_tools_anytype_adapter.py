...
Implements `IToolsProtocol` (AES403) and exports the `anytype` /
...
class AnytypeToolsAdapter(IToolsProtocol):
    """Anytype actions behind the tools protocol (AES403 implementor)."""
    ...
    def execute(
        ...
        """Dispatch *op* against this file's anytype/anytype-daemon units."""
        if spec is None:
            raise ToolUpdateError(f"anytype adapter got op={op!r} without a spec")
        ...
        raise ToolUpdateError(f"unsupported anytype adapter op {op!r}")
