...
``utility_launcher_writer``. Runs only after FR-001 success. One launcher per
binary plus one per manifest alias; MCP tools record ``mcp_binary``. A stale
foreign launcher (no provenance marker) is reported as a residual, never
overwritten; a correct launcher already in place is a no-op success.
"""
...
from modules.installer.src.contract_tool_installer_protocol import IToolLauncherRegistrar
