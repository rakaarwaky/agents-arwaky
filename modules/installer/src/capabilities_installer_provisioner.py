...
adapter's install/build sequence runs, with retry policy, partial-failure
cleanup, and the post-install version probe folded in. Never raises out of
``provision`` — every failure returns ``InstallResult(success=False, message)``.
"""
...
def _partial_cleanup(artifacts: list[Path]) -> None:
    """Remove half-built artifacts before reporting a failure (provisioning policy)."""
    for p in artifacts:
        try:
...
class ProvisionerCapability(IToolProvisioner):
    ...
    def provision(self, spec: ToolSpec, adapter: IToolAdapter, dry_run: bool = False) -> InstallResult:
        ...
        if adapter.satisfied(spec):
            return InstallResult(True, spec.id, "satisfied (no action needed)")

        try:
            artifacts = adapter.install(spec, root, daemons=self._daemons)
