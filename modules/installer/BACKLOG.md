...
| INS-02 | FR-002 | Registry-dispatch orchestrator (one capability per tool id) | P0 | Deprecated | `agent_installer_orchestrator.py` routes by id; no per-tool `if` branches; verified by import + `aa check` at `5556fd5`. The per-tool-id dispatch model is superseded: capabilities are now per business action (provisioner, launcher), and the orchestrator's registry keys on the manifest `runner` field selecting a leaf adapter. | @raka | None | 2026-09-19 |
