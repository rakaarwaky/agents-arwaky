"""ponytail uninstaller — port of tools/uninstall/uninstall_ponytail.py.

Removes ponytail's launchers + XDG data/config/cache artifacts via the
...
class PonytailUninstaller(IToolUninstaller):
    """Remove ponytail's launchers + XDG artifacts."""
    ...
    def uninstall(self, spec: ToolSpec) -> UninstallResult:
        print(f">>> Uninstalling ponytail...")

        remove_tool_artifacts("ponytail", LAUNCHERS)
        return UninstallResult(True, spec.id, "ponytail uninstalled (launchers + data + config + cache)")
