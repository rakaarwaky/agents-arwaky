"""codegraph uninstaller — port of tools/uninstall/uninstall_codegraph.py.

Removes codegraph's launchers + XDG data/config/cache artifacts via the
...
class CodegraphUninstaller(IToolUninstaller):
    """Remove codegraph's launchers + XDG artifacts."""
    ...
    def uninstall(self, spec: ToolSpec) -> UninstallResult:
        print(f">>> Uninstalling codegraph...")

        remove_tool_artifacts("codegraph", LAUNCHERS)
        return UninstallResult(True, spec.id, "codegraph uninstalled (launchers + data + config + cache)")
